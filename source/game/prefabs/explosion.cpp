...
void Explosion::setup()
{
	...
	settings_a.scale = 0.4 * get_scale().x;
	settings_a.particles_limit = 16;
	settings_a.lifetime = 0.5sec * get_scale().x;
	settings_a.spawn_interval = 0.0001sec;
	settings_a.texture = texture;
	settings_a.random_initial_velocity_min = 25 * get_scale().x;
	settings_a.random_initial_velocity_max = 65 * get_scale().x;
	...
	settings_b.scale = 0.5 * get_scale().x;
	settings_b.particles_limit = 32;
	settings_b.lifetime = 0.5sec * get_scale().x;
	settings_b.spawn_interval = 0.0001sec;
	settings_b.texture = texture;
	settings_b.random_initial_velocity_min = 75 * get_scale().x;
	settings_b.random_initial_velocity_max = 125 * get_scale().x;
	...
	settings_c.scale = 0.6 * get_scale().x;
	settings_c.particles_limit = 64;
	settings_c.lifetime = 0.6sec * get_scale().x;
	settings_c.spawn_interval = 0.0001sec;
	settings_c.texture = texture;
	settings_c.random_initial_velocity_min = 150 * get_scale().x;
	settings_c.random_initial_velocity_max = 200 * get_scale().x;
	...
	settings_d.scale = 0.8 * get_scale().x;
	settings_d.particles_limit = 64;
	settings_d.lifetime = 0.8sec * get_scale().x;
	settings_d.spawn_interval = 0.001sec;
	settings_d.texture = G_Resources_Storage.get_texture( "smoke_particle" );
	settings_d.random_initial_velocity_min = 150 * get_scale().x;
	settings_d.random_initial_velocity_max = 200 * get_scale().x;
